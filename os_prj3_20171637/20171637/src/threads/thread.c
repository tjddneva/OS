...
void
sema_thread_yield (void) 
{
  if(thread_started&&!intr_context()){
  struct thread *cur = thread_current ();
  enum intr_level old_level;
...
thread_set_nice (int new_nice) 
{
  struct thread *cur = thread_current();
  cur->nice = new_nice; 
  cur->priority = ((PRI_MAX*F) - (cur->recent_cpu / 4) - ((cur->nice * 2)*F))/F;
...
{ 
  int res = 100 * load_avg;
  if(res>0){
	res = (100*load_avg + F / 2) / F;
  }
  else if(res<0){
	res = (100*load_avg - F / 2) / F;
  }
...
{
  int res = 100 * thread_current()->recent_cpu;
  if(res>0){
	res = (100*thread_current()->recent_cpu + F / 2) / F;
  }
  else if(res<0){
	res = (100*thread_current()->recent_cpu - F / 2) / F;
  }
...
void 
recal_load_avg_and_recent_cpu(void)
{
  struct thread* t;
  struct list_elem* e;
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  sema_init(&(t->mylord),0);
  t->zomboid = false;
  sema_init(&(t->remember),0);
