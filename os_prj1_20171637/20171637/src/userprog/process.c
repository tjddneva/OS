...
process_wait (tid_t child_tid) 
{
  ...
  while (e != list_end(&(thread_current()->child))) {
	...
	for(element = list_begin(&thread_current()->child_status); element!=list_end(&thread_current()->child_status); element=list_next(element)){
		struct exit_status_item* item = list_entry(element, struct exit_status_item, elem);
		if(item->tid==child_tid){
  ...
  if (pd != NULL) 
    {
	  ...
	  struct exit_status_item* item = (struct exit_status_item*)malloc(sizeof(struct exit_status_item));
	  item->tid = cur->tid; item->exit_status = cur->exit_status;
	  list_push_back(&((cur->parent)->child_status),&(item->elem));
...
//  int j;
  char copy_name[256];
  strlcpy(copy_name,file_name,strlen(file_name)+1);
//  for(j=0; copy_name[j]!='\0' && copy_name[j]!=' '; i++);
//  copy_name[j] = '\0';
  char *save_ptr;
  char *cmd_name;
  cmd_name = strtok_r(copy_name," ",&save_ptr);
...
//  for (int i = 0, argu = strtok_r(copy_name2, " ", &save_ptr2); i < argc; i++, argu = strtok_r(NULL, " ", &save_ptr2)) {
   // len = strlen(token);
//    mystack[i] = argu;
//  }
